	...
	, input: {
		...
		, mask: {
			...
			, cep: function (obj, evt) {
				evt = evt || event;
				setTimeout(function () {
					obj.value = obj.value.replace(/\D/g, ""); //Remove tudo o que nÃ£o Ã© dÃ­gito
					obj.value = obj.value.replace(/(\d{5})(\d)/, "$1-$2"); //Coloca hÃ­fen entre o quarto e o quinto dÃ­gitos
				}, 1);
			}
