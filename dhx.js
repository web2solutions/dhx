...
var $dhx = {
				...
				if (node.readyState) {
					...
					if (url.indexOf(".css") != -1) {
						...
						node.onerror = function (e) {
							...
							//console.log("loaded  " + url);
							document.getElementById("$dhx_splash_div_file_info").innerHTML = '<br>error</b> when loading the file: <br>' + e.target.src.split("/")[e.target.src.split(
								"/").length - 1];
							//callback();
						};
					}
	...
	*/
	...
	, showDirections: function (m) {
		...
		if (m == "MSXML") {
			...
			template = template +
				'<b>Please click <a target="_blank" style="color:#003399;" href="http://www.microsoft.com/en-us/download/details.aspx?id=19662" title="download">here</a> to install the component or use Firefox or Google Chrome</b>';
		}
			...
			template = template +
				'You can also try to install Acrobat reader. <b>Please click <a target="_blank" style="color:#003399;" href="http://get.adobe.com/br/reader/" title="download">here</a> to download and install free Acrobat Reader</b>';
		}
	...
	, progressOn: function (m) {
		...
		//div_wrapper.style.opacity = "0.5";
		div_splash = document.createElement("DIV");
		div_splash.setAttribute("style"
			, 'font-family:Tahoma, Geneva, sans-serif; font-size:11px; color:#069; text-align:center; padding-top:30px;padding-left:10px;');
	...
	, toCurrency: function (num) {
		...
		for (var i = 0; i < Math.floor((num.length - (1 + i)) / 3); i++) num = num.substring(0, num.length - (4 * i + 3)) + ',' + num.substring(num.length - (4 * i +
			3));
		ret = num + '.' + cents;
		if (x == 1) ret = ' - ' + ret;
		return ret;
	...
	*/
	...
	, UTF8: {
		encode: function (s) {
			for (var c, i = -1, l = (s = s.split("")).length, o = String.fromCharCode; ++i < l; s[i] = (c = s[i].charCodeAt(0)) >= 127 ? o(0xc0 | (c >>> 6)) + o(0x80 |
					(c & 0x3f)) : s[i]);
			return s.join("");
		}
		, decode: function (s) {
	...
	, modifyDHTMLXloader: function () {
			...
			if (typeof dtmlXMLLoaderObject !== 'undefined') {
				dtmlXMLLoaderObject.prototype.loadXML = function (a, b, c, d) {
					...
					d ? (this.xmlDoc.setRequestHeader("User-Agent", "dhtmlxRPC v0.1 (" + navigator.userAgent + ")"), this.xmlDoc.setRequestHeader("Content-type", "text/xml")) :
						b && this.xmlDoc.setRequestHeader("Content-type", this.contenttype || "application/x-www-form-urlencoded");
...
$dhx.cookie = {
	...
	, del: function (sKey, sPath, sDomain) {
		...
		document.cookie = encodeURIComponent(sKey) + "=; expires=Thu, 01 Jan 1970 00:00:00 GMT" + (sDomain ? "; domain=" + sDomain : "") + (sPath ? "; path=" +
			sPath : "");
		return true;
	}
