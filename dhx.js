...
(function(namespace) {
    ...
    */
    ...
    createElement: function(c) {
        ...
        if (c.tag_name.toLowerCase() == 'canvas') {
            //if ($dhx.dhx_elements[id].width) console.log( $dhx.dhx_elements[id].width );
            if ($dhx.dhx_elements[id].width) element.width = $dhx.dhx_elements[id].width;
            if ($dhx.dhx_elements[id].height) element.height = $dhx.dhx_elements[id].height;
        }
        if (c.attributes) {
            for (var attribute in c.attributes) {
                element.setAttribute(attribute, c.attributes[attribute]);
            }
        }
        // attributes
