	...
	,form : function( c ){
		...
		dhxForm.fill = function( hash ){
			...
			dhxForm.fields.forEach(function( field ){
				if( field.type == 'input' && field.mask_to_use == 'br_currency' )
				{				
					if( hash[field.name] == '' || hash[field.name] == null )
						hash[field.name] = 0;
					hash[field.name] = new Number(parseFloat(hash[field.name])).formatMoney(2, '', '.', ',')
			...
			dhxForm.fields.forEach(function( field ){
				...
				{
					if( typeof hash[field.name] == 'undefined' )
						dhxForm.uncheckItem(field.name);
					else if( hash[field.name] == 1 )
						dhxForm.checkItem(field.name);
					else if( hash[field.name] == '1' )
						dhxForm.checkItem(field.name);
					else
						dhxForm.uncheckItem(field.name);
