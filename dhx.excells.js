...
$dhx.excells = $dhx.excells || {
	...
	, br: {
		currency: function (cell) {
			...
			this.setValue = function (val) {
				...
				this.setCValue("<span>R$ </span><span>" + new Number(parseFloat(val)).formatMoney(2, '', '.', ',') + "</span>", val);
			}
			this.getValue = function () {
	...
	, us: {
		currency: function (cell) { //excell name is defined here
			...
			this.setValue = function (val) {
				val = val || 0;
				this.setCValue("<span>USD </span><span>" + new Number(parseFloat(val.toString().replace(/,/g, ""))).formatMoney(2, '', ',', '.') + "</span>", val);
			}
			this.getValue = function () {
				var val = this.cell.childNodes[1].innerHTML;
				//val = val.toString().replace(/\./g,'');
