        ...
        **/
        addRoute: function(pattern, handler) {
            var route = router.addRoute(pattern, handler),
                self  = this;
            route.matched.add(function(url_payload){
			 	//console.log('XXXDDDDDD matched ', url_payload);
                self.url_payload = url_payload;
			});
			route.switched.add(function(url_payload){
			 	console.log('XXXDDDDDD switched: '+ url_payload);
			});
        },
        get: function( paramName ){
            var  self  = this;
            return paramName in self.url_payload ? self.url_payload[ paramName ] : null;
        },
        /**
