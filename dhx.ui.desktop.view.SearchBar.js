$dhx.ui.desktop.view.SearchBar = {
	...
	, _search_bar: function () {
		...
		self.search_bar.ondblclick = function(){
			$dhx.ui.desktop.view.SearchBar.open();
		}
	}
	...
	, _input_search: function () {
		...
		self.input_search.ondblclick = function (e) {
	...
	, _dataview_results: function (schema) {
		...
		self.dataview_results = new dhtmlXDataView({
			...
			, type: {
				template: "#name# - #record_id# <br><br>location: <br><img width='46' height='46' src='" + $dhx.ui.cdn_address + '' +$dhx.ui.desktop.database + "/private/T-RexWebOS/icons/" + "" + "/#icon#'/><br>#program#"
				, height: 100
				, width: 300
			}
			, tooltip: {
				template: "double click to open <br>#name#<br>Item id: #record_id#<br>#program#"
			}
		});
		self.dataview_results.attachEvent("onItemClick", function (itemId) {
			//alert(itemId);
			
			console.log(  self.dataview_results.get(itemId) )
			
		});
		
		self.dataview_results.attachEvent("onItemDblClick", function (itemId) {
			//alert(itemId);
			
			var hash = self.dataview_results.get(itemId);
			...
			var event = new MouseEvent('click', {
			  ...
			  //'$dhx.ui.desktop.active_area.side_button.' + c.appId
			  
			  var cb = document.getElementById('$dhx.ui.desktop.active_area.side_button.' + hash.appId); 
	...
	, doSearch: function (value) {
		...
		$dhx.ui.desktop.Registry.cruders.forEach(function (cruder, index, array)
		{
			if (cruder.column_to_search_id) {
				...
				cruder.column_to_search_index.toString().split(', ').forEach( function( column_name, index ){
					...
					, onReady: function (records, tx, event) {
						...
						records.forEach(function (recordset, index, array) {
							...
							cruder.column_to_search_index.toString().split(', ').forEach( function( column_name, index ){
								...
								recordset['name'] += recordset[column_name] + ' ';
							} )
							//console.log( recordset['name'] );
							
							//recordset['name'] = recordset[cruder.column_to_search_index.toString().split(', ')[0]];
							recordset['collection'] = cruder.collection;
							recordset['icon'] = cruder.icon;
							recordset['program'] = cruder.appName;
							recordset['appId'] = cruder.appId;
