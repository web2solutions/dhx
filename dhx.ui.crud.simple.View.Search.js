...
$dhx.ui.crud.simple.View.Search = {
    ...
    _form: function(uid, db_settings, schema, controllerAppId) {
		...
		for( var index = 0; index < db_settings.form.template.length; index++)
		{
			var field = db_settings.form.template[ index ];
			if( field.type == 'vault' )
			{
				continue;	
			}
			if( field.mask_to_use.toString().indexOf('currency') != -1 )
			{
				continue;	
			}
		...
		$dhx.ui.crud.simple.View.settings.Search.form.template[1].list = fields;

		
		self.form[ uid ] = new $dhx.component.form({
			id: form_id
			, parent: self.layout[uid].cells('a')
			, settings: $dhx.ui.crud.simple.View.settings.Search.form
		});
